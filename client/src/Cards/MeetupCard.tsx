...
const MeetupCard = ({ meetup }) => {
  ...
  return (
    ...
    >
    <div className={`${!isOutDated ? 'hidden' : 'flex'} absolute  bg-zinc-50/30 top-0 left-0 w-full h-full z-50`}></div>
    <span className="absolute top-2 right-2 border shadow-md font-semibold text-xs bg-zinc-50 rounded-full px-2 py-0.5">{meetup.meetup_theme}</span>
      <img
      ...
      <div className="p-2 h-40  w-full flex flex-col justify-between">
        ...
        <div className="flex gap-2 ">   
         ...
         <span><FaArrowRight/></span>
          ...
          >
           Me apunto!
         <span><FaPersonWalkingArrowRight/></span>  </a>
        </div>
      </div>
      ...
      <div className="flex justify-between items-center px-2 border-t  py-1">
        ...
        >
          <img
            ...
            alt="user picture"
          />
          <p className="font-medium text-sm">{meetup.main_user_details.username}</p>
        </div>
      </div>
      </div>
    </div>
